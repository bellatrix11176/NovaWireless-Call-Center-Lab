...
def main() -> int:
    ...
    if n_mismatch > 0 and len(eligible_idx) >= 2:
        # Pre-build customer → eligible index lookup using vectorized groupby
        if args.within_customer_only:
            from collections import defaultdict
            cust_to_eligible: dict = defaultdict(list)
            eligible_df = out.loc[eligible_idx, ["customer_id"]]
            for cust_id, grp in eligible_df.groupby("customer_id"):
                cust_to_eligible[cust_id] = grp.index.tolist()
        ...
        for i in chosen:
            ...
            if args.within_customer_only:
                pool = np.array([x for x in cust_to_eligible[cust] if x != int(i)])
            else:
