...
This lab makes that divergence measurable. Every call carries both the **proxy label** (what the agent reported) and the **ground truth** (what actually happened). That gap is where governance begins.
...
| `gamed_metric` | 10% | Rep marks resolved without fixing — the core gaming signal |
...
|---|---|
| `resolution_flag` | What the rep reported — the proxy KPI |
