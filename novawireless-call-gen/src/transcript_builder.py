...
def _body_loyalty_offer_missed(customer: dict, credit_info: dict,
                               rng: np.random.Generator) -> List[Turn]:
    """
    Customer calls shopping for a competitive offer — competitor is advertising
    free phones for new customers. Rep offers a band-aid temporary discount
    instead of the trade-in loyalty upgrade (phone + 24-month EIP) that would
    have retained the customer long-term. Call gets marked resolved. Customer
    calls back when discount expires or churns during Q4 holiday season.
    """
    charge  = round(float(customer.get("monthly_charges", 85.0)), 2)
    applied = credit_info.get("credit_applied", False)
    amount  = credit_info.get("credit_amount", 10.0)

    # Two band-aid variants: $10/6mo or $20/12mo
    if amount >= 18:
        discount_offer = f"${amount:.0f} off per month for the next 12 months"
        discount_total = f"${amount * 12:.0f}"
    else:
        discount_offer = f"${amount:.0f} off per month for the next 6 months"
        discount_total = f"${amount * 6:.0f}"

    if applied:
        return [
            {"speaker": "Customer", "text": "I've been with NovaWireless for years and I keep seeing ads where competitors are giving brand new phones away for free to new customers. I'm paying full price and staying loyal and I feel like I'm getting nothing out of it."},
            {"speaker": "Agent",    "text": "I completely understand that frustration and I really appreciate your loyalty. Let me take a look at your account and see what I can do for you today."},
            {"speaker": "Agent",    "text": f"I can see you've been with us for {customer.get('tenure_months', 24)} months and your account is in great standing. Let me check what promotions are available for existing customers."},
            {"speaker": "Customer", "text": "I was hoping maybe I could get a deal on upgrading my phone. I saw one of those trade-in offers but I wasn't sure if I qualified."},
            {"speaker": "Agent",    "text": f"What I'm able to offer you today is a loyalty discount of {discount_offer}. That would save you {discount_total} over that period. I can apply that to your account right now if you'd like."},
            {"speaker": "Customer", "text": "That's it? I mean I appreciate it but the other carrier is literally giving away a free phone. I was hoping to upgrade."},
            {"speaker": "Agent",    "text": f"I hear you, and I want to be upfront — I'm not seeing an active trade-in promotion I can apply to your account right now through my system. The {discount_offer} is the best I have available today. Would you like me to go ahead and apply it?"},
            {"speaker": "Customer", "text": "I guess so. But I want you to put a note that I'm interested in upgrading. I might just go somewhere else if something doesn't change."},
            {"speaker": "Agent",    "text": f"Absolutely, I'm noting that on your account right now. I've applied the {discount_offer} — you'll see that starting on your next bill. And I've flagged you as upgrade-interested so if anything comes up our retention team may reach out."},
            {"speaker": "Customer", "text": "Okay. I'll give it some time. But not forever."},
            {"speaker": "Agent",    "text": "I completely understand. Thank you for staying with us and for letting us know. We do value your loyalty."},
        ]
    else:
        return [
            {"speaker": "Customer", "text": "I've been a customer for years and I keep seeing ads where other carriers give free phones to new customers. What are you doing for loyal customers like me?"},
            {"speaker": "Agent",    "text": "I appreciate you bringing that up and I definitely understand the frustration. Let me look at your account and what's currently available."},
            {"speaker": "Agent",    "text": f"I can see you've been with us for {customer.get('tenure_months', 24)} months — that's great. Let me check what I have available for you."},
            {"speaker": "Customer", "text": "I was thinking about maybe doing a trade-in upgrade. I have an older phone I'd trade in. Does NovaWireless have something like that?"},
            {"speaker": "Agent",    "text": "We do have trade-in programs from time to time. Right now I'm not seeing an active offer I can apply directly on this call, but I can document your interest and have someone from our upgrade team follow up with you. Sometimes those offers aren't in my system but our specialists have access to additional promotions."},
            {"speaker": "Customer", "text": "So there's nothing you can do for me today?"},
            {"speaker": "Agent",    "text": "I want to be honest with you — I don't have a trade-in promotion I can apply right now from my end. I've submitted a note on your account flagging your interest in an upgrade. If a retention specialist reaches out, they'll have more tools available. I'm sorry I can't do more on this call."},
            {"speaker": "Customer", "text": "Okay. Well I hope someone actually calls because I'm starting to look at other options seriously."},
            {"speaker": "Agent",    "text": "I've documented that clearly on your account. Thank you for giving us the chance to make this right. We really do value your loyalty."},
        ]
...
def build_transcript(
    ...
    elif scenario == "loyalty_offer_missed":
        turns += _body_loyalty_offer_missed(customer, credit_info, rng)
        turns += _closer_clean(agent_name, customer_name, rng)
