...
TRUE_RESOLUTION_PROBS = {
    ...
    "loyalty_offer_missed": 0.14,  # band-aid discount ≠ durable retention
}
...
PROXY_RESOLUTION_PROBS = {
    ...
    "loyalty_offer_missed": 0.91,  # rep marks resolved after applying discount
}
...
REPEAT_30D_PROBS = {
    ...
    "loyalty_offer_missed": 0.18,  # calls back within 30d asking again
}
...
REPEAT_31_60D_PROBS = {
    ...
    "loyalty_offer_missed": 0.58,  # discount expires → calls back weeks later
}
...
SIGNAL_PROBS = {
    ...
    "loyalty_offer_missed": (0.01, 0.00, 0.05, 0.00, 0.00),
}
...
REP_AWARE_PROBS = {
    ...
    "loyalty_offer_missed": 0.25,  # some reps know the trade-in offer exists but don't push it
}
...
AHT_MULTIPLIERS = {
    ...
    "loyalty_offer_missed": 0.85,  # short call — discount applied quickly, customer not fully satisfied
}
...
SCENARIO_CALL_TYPE = {
    ...
    "loyalty_offer_missed": "Promotion Inquiry",
}
...
SCENARIO_SUBREASON = {
    ...
    "loyalty_offer_missed": "Loyalty upgrade not offered",
}
...
SCENARIO_CHURN_MULTIPLIERS = {
    ...
    "loyalty_offer_missed": 1.80,  # highest churn multiplier — competitor offer never countered
}
...
SCENARIO_TRUST_DECAY = {
    ...
    "line_add_legitimate":  0.00,
    "loyalty_offer_missed": 0.14,  # trust erodes each time customer is fobbed off
}
...
CREDIT_CONFIG = {
    ...
    "gamed_metric_aware":   (0.80, "bandaid",         False, 10.0, 20.0),
    ...
    "fraud_hic_exchange":   (0.95, "fee_waiver",      True,  35.0, 75.0),
    "fraud_care_promo":     (0.00, "none",            True,   0.0,  0.0),
    ...
    # Band-aid discount: $10/6mo or $20/12mo — authorized but not durable retention
    "loyalty_offer_missed": (0.88, "bandaid",         True,  10.0, 20.0),
}
...
# Full scenario mix including fraud scenarios — used for standard simulation
FULL_SCENARIO_MIX = {
    "clean":                0.36,
    "unresolvable_clean":   0.07,
    "gamed_metric":         0.13,
    "fraud_store_promo":    0.06,
    "fraud_line_add":       0.06,
    "fraud_hic_exchange":   0.06,
    "fraud_care_promo":     0.05,
    "activation_clean":     0.05,
    "activation_failed":    0.03,
    "line_add_legitimate":  0.02,
    "loyalty_offer_missed": 0.11,  # on every other call in Q3/Q4
}

# Goodhart scenario mix — governance-aligned, no overt fraud
# loyalty_offer_missed stays because it's a systemic failure even in the "after" world
GOODHART_SCENARIO_MIX = {
    "clean":                0.42,
    "unresolvable_clean":   0.10,
    "gamed_metric":         0.18,
    "activation_clean":     0.07,
    "activation_failed":    0.04,
    "line_add_legitimate":  0.06,
    "loyalty_offer_missed": 0.13,  # still present — governance detects it via trust decay + repeat
}
...
def assign_scenario(rng: np.random.Generator,
                    scenario_mix: dict,
                    rep_state: dict | None = None) -> str:
    """
    Sample a scenario from the provided mix dict.
    rep_state is accepted for API compatibility but not used in sampling.
    """
