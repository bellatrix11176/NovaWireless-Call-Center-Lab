...
VALID_CREDIT_TYPES_PER_SCENARIO = {
    ...
    "loyalty_offer_missed": {"none", "bandaid"},
}
...
CALL_TYPE_MAP = {
    ...
    ("loyalty_offer_missed", "*"): ("Promotion Inquiry", "Loyalty upgrade not offered"),
...
def parse_args():
    ...
    ap.add_argument("--month", default=None,
                    help="Month tag e.g. 2025-01 — selects specific metadata file.")
...
def main() -> int:
    ...
    if args.meta:
        ...
        # Look in output/call-gen/metadata/ first, then flat output/ as fallback
        candidates = sorted((outdir / "call-gen" / "metadata").glob("calls_metadata*.csv"))
        if not candidates:
            candidates = sorted(outdir.glob("calls_metadata*.csv"))
        if not candidates:
            print("ERROR: No calls_metadata*.csv found in output/call-gen/metadata/ or output/")
            return 1
        if args.month:
            candidates = [c for c in candidates if args.month in c.stem]
            if not candidates:
                print(f"ERROR: No calls_metadata file found for month {args.month}")
                return 1
        meta_p = candidates[-1]
    ...
    if args.jsonl:
        ...
        jsonl_p = outdir / "call-gen" / "transcripts" / f"{stem}.jsonl"
    ...
    if args.out:
        ...
        out_p = outdir / "call-gen" / "sanitized" / f"{stem}.csv"
